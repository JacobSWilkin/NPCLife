...
// -----------------------------------------------------------------------------------------------
cbuffer SkinningConstants: register(b5)
{
...
v2p_t VertexMain(vs_input_t input)
{
    ...
    // ----------------------------------- GPU SKINNING --------------------------------------//
    float4 skinnedPosition = modelPosition;

    skinnedPosition += input.boneWeights.x * mul(BoneMatrices[input.boneIndices.x], modelPosition);
    skinnedPosition += input.boneWeights.y * mul(BoneMatrices[input.boneIndices.y], modelPosition);
    skinnedPosition += input.boneWeights.z * mul(BoneMatrices[input.boneIndices.z], modelPosition);
    skinnedPosition += input.boneWeights.w * mul(BoneMatrices[input.boneIndices.w], modelPosition);

	float4 worldPosition = mul(ModelToWorldTransform, skinnedPosition);
	...
	skinnedTangent += input.boneWeights.x * mul(BoneMatrices[input.boneIndices.x], float4(input.modelTangent, 0.0f));
	skinnedTangent += input.boneWeights.y * mul(BoneMatrices[input.boneIndices.y], float4(input.modelTangent, 0.0f));
	skinnedTangent += input.boneWeights.z * mul(BoneMatrices[input.boneIndices.z], float4(input.modelTangent, 0.0f));
	skinnedTangent += input.boneWeights.w * mul(BoneMatrices[input.boneIndices.w], float4(input.modelTangent, 0.0f));

	skinnedBitangent += input.boneWeights.x * mul(BoneMatrices[input.boneIndices.x], float4(input.modelBitangent, 0.0f));
	skinnedBitangent += input.boneWeights.y * mul(BoneMatrices[input.boneIndices.y], float4(input.modelBitangent, 0.0f));
	skinnedBitangent += input.boneWeights.z * mul(BoneMatrices[input.boneIndices.z], float4(input.modelBitangent, 0.0f));
	skinnedBitangent += input.boneWeights.w * mul(BoneMatrices[input.boneIndices.w], float4(input.modelBitangent, 0.0f));

	skinnedNormal += input.boneWeights.x * mul(BoneMatrices[input.boneIndices.x], float4(input.modelNormal, 0.0f));
	skinnedNormal += input.boneWeights.y * mul(BoneMatrices[input.boneIndices.y], float4(input.modelNormal, 0.0f));
	skinnedNormal += input.boneWeights.z * mul(BoneMatrices[input.boneIndices.z], float4(input.modelNormal, 0.0f));
	skinnedNormal += input.boneWeights.w * mul(BoneMatrices[input.boneIndices.w], float4(input.modelNormal, 0.0f));

	// ----------------------------------------------------------------------------------------//

	float4 worldTangent   = mul(ModelToWorldTransform, skinnedTangent);
	float4 worldBitangent = mul(ModelToWorldTransform, skinnedBitangent);
	float4 worldNormal    = mul(ModelToWorldTransform, skinnedNormal);
